...
```{r }
forward2<- glm(formula = y ~ duration + job + contact + day_of_week + default + previous+ pdays, family = binomial(link = "logit"), data = balanced_train)
forward2
```
...
```{r }
logr<-glm(formula = y ~ duration + job + contact + day_of_week + default + previous+ pdays, family = binomial(link = "logit"), data = balanced_train)
...
```{r }
dat.train.x <- as.data.frame(balanced_train)
dat.train.y <- balanced_train$y
dat.train.y <- as.factor(as.character(dat.train.y))
dat.test.x<-as.data.frame(balanced_test)
dat.test.y<- balanced_test$y
dat.test.y <- as.factor(as.character(dat.test.y))

logr1<-glm(formula = y ~ duration + job + contact + day_of_week + default + previous+ pdays, family = binomial(link = "logit"), data = balanced_train)
...
```{r }
logr<- glm(y ~ job + education + default + contact +duration + poutcome + campaign + month, family = binomial(link = "logit"),data = balanced_train)
...
logr2<-glm(y ~ job + education + default + contact +duration + poutcome + pdays + campaign, family = binomial(link = "logit"),data = balanced_train)
...
```{r }
backwardfinal<- glm(formula = y ~ job + education + default + contact +duration + pdays + campaign + poutcome, family = binomial(link = "logit"),data = balanced_train)
backwardfinal
```
...
```{r }
logr<-glm(y ~ job + education + default + contact +duration + pdays + campaign + poutcome, family = binomial(link = "logit"),data = balanced_train)
logr.probs<-predict(logr, newdata=balanced_test)
ME<-c()
FP<-c()
FN<-c()
...
logr3<- glm(y ~ job + education + default + contact +duration + pdays + campaign + poutcome, family = binomial(link = "logit"),data = balanced_train)
logr.probs3<-predict(logr3, newdata=balanced_test)
